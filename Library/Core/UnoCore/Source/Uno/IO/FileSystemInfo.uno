...
namespace Uno.IO
{
    public abstract class FileSystemInfo
    {
        ...
        protected FileSystemInfo(string originalPath)
        {
            ...
            _fullPath = Path.GetFullPath(_originalPath);
        }
        ...
        extern(DOTNET) private System.IO.FileSystemInfo _status;

        extern(DOTNET) internal virtual System.IO.FileSystemInfo LoadStatus()
        {
            // Marking this as abstract combined with extern did not work, why?
            throw new InvalidOperationException();
        }

        extern(DOTNET) internal System.IO.FileSystemInfo Status
        {
            get
            {
                if (_status == null)
