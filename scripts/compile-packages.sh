...
UNO="$ROOT/prebuilt/uno.exe"
PACKAGES="$ROOT/Library/Core/build"
ARGUMENTS=""
...
function usage {
    ...
    echo "    -v, --verbose                        verbose Uno builds (Passes -v to Uno. For even more verbosity, use -a -vv etc.)"
}
...
echo
monow "$UNO" build --target=$TARGET --no-strip --clean $ARGUMENTS $OUTPUT_DIR
